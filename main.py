    ...
    if command == 'exit':
        ...
        if shopinput[0] == '1':
            ...
            if player['coins'] >= 10 * int(shopinput[1]):
                ...
                player['boosters'] += int(shopinput[1])
                print(f"{Fore.GREEN + Style.BRIGHT}✔ Вы успешно купили кликерыы, кол-во: {shopinput[1]}! Он работает 5 минут, для его включения напишите clicker{Style.RESET_ALL}")
            else:
                shopcancel = 1
